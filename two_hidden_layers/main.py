...
FFMPEG_PATH = r'C:\Users\張家銘\Desktop\FINAL_VERSION\FFMPEG\bin\ffmpeg.exe' # for recording the gameplay
VIDEO_SAVE_PATH = r'C:\Users\張家銘\Desktop\FINAL_VERSION\test_training\TEST3_REPORT_DEEP_TRAIN'
#===============================================================================================================#
def initialize_snake_layers(snake:snakegame.Snake) -> None:
    ...
    hiddenlayer1 = neuralnet.Layer(12,activation_func=ActivationFunction.tanh)
    hiddenlayer2 = neuralnet.Layer(8,activation_func=ActivationFunction.tanh)
    outputlayer = neuralnet.Layer(4,activation_func=ActivationFunction.linear)
    neuralnet.construct_layers(layers_list=[inputlayer,hiddenlayer1,hiddenlayer2,outputlayer],seed=SEED)
    snake.layers = {"Input":inputlayer,"Hidden1":hiddenlayer1,"Hidden2":hiddenlayer2,"Output":outputlayer}
...
def decide_direction(snake:snakegame.Snake) -> str: # return direction
    ...
    neuralnet.forward_propogation(snake.layers["Hidden1"],snake.layers["Hidden2"],batch_normalize=True,gamma=GAMMA,beta=BETA)
    neuralnet.forward_propogation(snake.layers["Hidden2"],snake.layers["Output"],batch_normalize=True,gamma=GAMMA,beta=BETA)
...
def reproduce(snakelist,gameplay_result):
    ...
    for ID in range(POPULATION_SIZE//2):
        ...
        for layer_name in ["Input","Hidden1","Hidden2"]:
            ...
            for ID in range(POPULATION_SIZE):
                if ID in top_index[-NUMBER_OF_VIDEOS:]:
                    plot_parameters={
                        ...
                        "layerstructure": "[16-tanh-12-tanh-8-tanh-4]",
